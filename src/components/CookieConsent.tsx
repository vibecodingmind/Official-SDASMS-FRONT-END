...
export default function CookieConsent() {
  ...
  const handleAccept = () => {
    setMounted(false)
    // Wait for exit animation before setting consent
    setTimeout(() => setConsentAndNotify('accepted'), 300)
  }

  const handleDecline = () => {
    setMounted(false)
    setTimeout(() => setConsentAndNotify('declined'), 300)
  }

  const handleDismiss = () => {
    setMounted(false)
    setTimeout(() => setConsentAndNotify('dismissed'), 300)
  }
  ...
  return (
    <div
      className={`fixed bottom-0 left-0 right-0 z-50 bg-white dark:bg-[#1A0A2E] border-t border-gray-200 dark:border-white/10 shadow-2xl transition-transform duration-300 ease-out ${
